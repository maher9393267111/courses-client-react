...
const handleSubmit = async(e) => {
    ...
    API.post('/createParentCat', { name,image:imageobje }).then((res) => {
        console.log('res', res);
        setImageshow(res.data.parentcat.image.secure_url);

        toast.success(`Parent Category${res.data.parentcat.name} Created Successfully`);
    }
...
setTimeout(() => {
    ...
    API.get('/allParentCats').then((res) => {
        console.log('res', res);
...
<Box p={4} color='white' h={'70px'}  paddingLeft={'30px'} fontSize={'22px'}  w={'100%'} bg='tomato'> Parent Category Create</Box>
